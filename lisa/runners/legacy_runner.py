...
class LegacyRunner(BaseRunner):
    ...
    def run(self) -> None:
        ...
        for index, config in enumerate(lisav2_configurations):
            ...
            git.clone(
                ...
                dir_name=self._get_dir_name(index),
            )

        # start run for each configuration
        run_in_threads(
            [
                partial(self._start_sub_test, index, config)
                for index, config in enumerate(lisav2_configurations)
            ],
    ...
    def _start_sub_test(self, index: int, configuration: schema.LegacyTestCase) -> None:
        """
        entry point of each LISAv2 process.
        """
        # start LISAv2 process
        code_path = self._working_folder / self._get_dir_name(index)
        ...
        # track test progress
        log = get_logger(id_=str(index), parent=self._log)
        try:
            _track_progress(
                process=process, working_dir=code_path, log=log, runner=self
            )
        finally:
    ...
    def _get_dir_name(self, index: int) -> str:
        return f"repo_{index}"
...
class ResultStateManager:
    ...
    def __init__(self, log: Logger) -> None:
        self._results: List[TestResultMessage] = []
        self.log = log
    ...
    def _extend_all_results(self, all_cases: List[Dict[str, str]]) -> None:
        ...
        if len(self._results) < len(all_cases):
            for i in range(len(self._results), len(all_cases)):
                ...
                new_result = TestResultMessage(
                    id_=len(self._results),
                    name=all_cases[i]["name"],
                    environment_information={},
                )
                self._results.append(new_result)
                notifier.notify(new_result)
    ...
    def _is_matched_infomation(
        self, result: TestResultMessage, information: Dict[str, str]
    ) -> bool:
        ...
        if (
            "image" not in result.environment_information
            or "location" not in result.environment_information
        ):
        ...
        result_vmsize = result.environment_information.get("vmsize", "")
        ...
        result_key = self._get_case_key(
            result.name,
            result.environment_information["image"],
            result.environment_information["location"],
            result_vmsize,
        )
    ...
    def _set_result(
        ...
        if message:
            # replace message to get latest information if there is.
            result.message = "\n".join([result.message, message])
        result.environment_information.update(information)
        if result.status != status:
            image = result.environment_information.get("image", "")
            location = result.environment_information.get("location", "")
            vmsize = result.environment_information.get("vmsize", "")
            self.log.info(
...
class LogParser(InitializableMixin):
    ...
    def _open_log(self) -> None:
        """
        V2 opens log file frequently to write content. open may be failed due to this.
        So retry to make it more stable.
        """
        if not self._file:
            self._file = open(self._runner_log_path, "r")
...
def _track_progress(
    process: Process, working_dir: Path, log: Logger, runner: LegacyRunner
) -> None:
    ...
    if process.is_running() and not runner.canceled:
        case_states = ResultStateManager(log=log)
