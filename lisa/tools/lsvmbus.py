...
from lisa.operating_system import Debian, Linux, Redhat, Suse
...
class ChannelVPMap:
    def __init__(self, channels_raw: str) -> None:
        self.parse(channels_raw)

    def parse(self, raw_str: str) -> None:
        matched_channel = PATTERN_CHANNEL.match(raw_str)
        if matched_channel:
            self.rel_id, self.target_cpu = (
...
class VmBus:
    ...
    def parse(self, raw_str: str) -> Any:
        ...
        raw_channels = re.finditer(PATTERN_CHANNEL, raw_str)
        for channel in raw_channels:
            matched_channels = PATTERN_CHANNEL.match(channel.group())
            if matched_channels:
                channel_vp_map = ChannelVPMap(matched_channels.group())
...
class Lsvmbus(Tool):
    ...
    def install(self) -> bool:
        ...
        elif isinstance(linux_os, Debian):
