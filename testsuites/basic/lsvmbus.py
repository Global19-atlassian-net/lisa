...
class LsVmBus(TestSuite):
    @TestCaseMetadata(
        description="""
        This test is to check channel count of each netvsc
        and storvsc SCSI device.
        When count is unexpected, then test fail.
        """,
        priority=1,
    ...
    def lsvmbus_channel_counting(self) -> None:
        ...
        for vmbus_name in vmbus_class.names:
            asserts.assert_in(vmbus_name, actual_vmbus_names)
        ...
        for vmbus in vmbus_list:
            if vmbus.vmbus_name == "Synthetic network adapter":
                asserts.assert_equal(
                    expected_network_channel_count, len(vmbus.channel_vp_map)
                )
            if vmbus.vmbus_name == "Synthetic SCSI Controller":
                asserts.assert_equal(
                    expected_scsi_channel_count, len(vmbus.channel_vp_map)
                )
